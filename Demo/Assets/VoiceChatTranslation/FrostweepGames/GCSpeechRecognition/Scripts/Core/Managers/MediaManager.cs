...
namespace FrostweepGames.Plugins.GoogleCloud.SpeechRecognition
{
    public class MediaManager : IService, IMediaManager
    {
		...
		public void StartRecord(bool withVoiceDetection = false, AudioClip nonVoiceAudio = null)
		{
			if (IsRecording)
				return;
			...
			if(nonVoiceAudio != null)
			{
				_microphoneWorkingAudioClip = nonVoiceAudio;
			}
			else
			{
		...
		public void StopRecord()
		{
			...
			{
				LastRecordedRaw = _currentRecordingVoice.ToArray();
				LastRecordedClip = AudioConvert.Convert(LastRecordedRaw, _microphoneWorkingAudioClip.channels, 16000);			//_microphoneWorkingAudioClip.channels
			}
		...
		{	
			...
			_previousSamplePosition = _currentSamplePosition;
			if(_currentRecordingVoice.Count == 0)
			{	
				_currentRecordingVoice.AddRange(_currentAudioSamples);
			}
		}
	}
}